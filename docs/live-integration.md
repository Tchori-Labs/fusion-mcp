...
- the HTTP server accepts its loopback connection but refuses connections to
  every non-internal IPv4 address on the host; when no such address exists, the
  probe emits a clear skip note to stderr;
- SIGTERM closes active HTTP sessions and exits cleanly without writing to
  stdout;
...
- [ ] Live stdio and HTTP transport journeys are green for 10 consecutive
      iterations.
- [ ] The HTTP journey confirms the port accepts loopback only; any skipped
      non-loopback probe is reviewed and justified by the host having no
