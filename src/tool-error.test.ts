...
describe("tool error contract", () => {
  ...
  it("allows only schema-derived validation path fields", () => {
    const hostileNumericPath = 987654321;
    const result = formatValidationError(
      [
        {
          path: ["id", secretMarker, hostileNumericPath],
          message: `id contained ${secretMarker}`,
        },
        {
          path: [bodyMarker, Symbol(secretMarker), hostileNumericPath],
          message: bodyMarker,
        },
      ],
      new Set(["id"]),
    ...
    expect(rendered).not.toContain(secretMarker);
    expect(rendered).not.toContain(bodyMarker);
    expect(rendered).not.toContain(String(hostileNumericPath));
  });
