  ...
  return errorResult({
    error: {
      ...
      details: issues.map((issue) => ({
        // Zod permits custom/refinement paths to contain received argument
        // values. Only schema-derived field names are safe to serialize;
        // numeric indices and every other path segment are redacted.
        path: (issue.path ?? []).flatMap((part) =>
          typeof part === "string" && allowedPathSegments.has(part)
            ? [part]
            : [],
        ),
        // Zod also permits custom/refinement messages, which may contain
        // received argument values. Never serialize those untrusted messages.
