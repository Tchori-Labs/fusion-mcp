...
describe("governed tool error envelopes", () => {
  ...
  it.each([
    { label: "null", suppliedArguments: null },
    { label: "array", suppliedArguments: [unsafeMarker] },
  ])(
    "normalizes $label argument containers before strict SDK parsing",
    async ({ suppliedArguments }) => {
      const fetchMock = vi.fn<FetchLike>();
      const harness = await createHarness(
        parseConfig({ FUSION_TOKEN: tokenMarker }),
        { fetch: fetchMock },
      ...
      try {
        const result = await harness.client.callTool({
          name: "get_task",
          arguments: suppliedArguments as unknown as Record<string, unknown>,
        });
  ...
  it("returns invalid_upstream_payload for an empty successful read", async () => {
    const fetchMock = vi
      .fn<FetchLike>()
