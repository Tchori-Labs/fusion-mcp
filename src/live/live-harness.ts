...
export async function assertNoNonLoopbackListener(
  port: number,
  timeoutMs = 250,
): Promise<void> {
  const addresses = [
  ...
  if (addresses.length === 0) {
    process.stderr.write(
      "fusion-mcp live bind probe skipped: no non-internal IPv4 address is available.\n",
    );
    return;
  }
  ...
  for (const address of addresses) {
    await new Promise<void>((resolve, reject) => {
      ...
      socket.once("connect", () => {
        finish(
          new Error(
            `HTTP MCP server accepted a non-loopback connection on ${address}:${port}`,
          ),
        );
      });
