...
**Implementation status:** `get_board_health` and `list_tasks` are implemented.
The remaining tools are delivered by tasks FM-001 … FM-004 (see `briefs/`) on
top of the existing `FusionClient`.
